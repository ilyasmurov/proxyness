...
**Server picker (config-driven list, Auto by default — 1.47.0, PRXNS-20/21/22)**: the live exit list comes from the config service: `/api/client-config` carries `servers: [{id,label,addr}]` (stored as JSON under `service_config.servers`, edited on the admin **Servers** page, validated on write). The main process caches the whole config on disk, so the renderer has the list even on an offline start; `BUILTIN_SERVERS` in `App.tsx` (Aeza NL `178.236.252.28:443`, "Aeza via RU" bridge `157.22.194.55:4443`, raw IPs — the client never resolves DNS) is only the fallback for an empty config. A non-empty config list replaces it wholesale; list order = Auto priority after the last live server. The Settings → General "Proxy Server" switch offers `Auto | <each server>`; a manual pick whose id vanished from the list falls back to Auto. `serversFor(list, choice, lastGood)` builds the dial list (Auto = every address with `localStorage["proxyness-last-server"]` first, fed by the `server` field of `/tun/status`; manual = one address, pinned). The list goes to the daemon as `servers` on `/connect` and `/tun/start`; the daemon's `transport.ServerRing` does the failover (decisions.md). Choice persists under `localStorage["proxyness-server-v2"]`.
...
- Pages: Dashboard (SSE-driven), Users, UserDetail, Sites, SiteDetail, Notifications, **Servers** (exit list sent to clients: id/label/addr, kind exit|bridge, via; plus the Taskless egress proxy URL and the `hosts` map of machine labels/extras for the Topology page — PRXNS-21/23), **Topology** (live traffic map grouped by physical machine, PRXNS-23: polls `GET /admin/api/topology` every 15 s; the control plane's server probes exits/bridges over their admin API, the config service, Postgres, DNS and the egress proxy — see `server/internal/topology`. The grouping is derived in the browser: a node's machine is the host of its address, the control plane's is the IP its DNS name resolves to, the replica card is the `replica:<exit>` check; machine names and probe-less info rows come from `service_config.hosts`, edited in the Servers page's "Machines" table), Releases, Changelog, Logs
