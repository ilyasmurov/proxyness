...
**Server picker (two exits, Auto by default — 1.46.0, PRXNS-20)**: `SERVERS` in `App.tsx` lists both exits (Serverspace NL `188.227.86.205:443`, Aeza NL `178.236.252.28:443`, raw IPs — the client never resolves DNS). The Settings → General "Proxy Server" switch offers `Auto | Serverspace NL | Aeza NL`. `serversFor(choice, lastGood)` builds the dial list: Auto = every address with the last live one first (`localStorage["proxyness-last-server"]`, fed by the `server` field of `/tun/status`), a manual pick = that one address, pinned. The list goes to the daemon as `servers` on `/connect` and `/tun/start`; the daemon's `transport.ServerRing` does the actual failover (see decisions.md). Choice persists under `localStorage["proxyness-server-v2"]`; the old `proxyness-server` key is removed at module load.
