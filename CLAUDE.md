...
- **Postgres 16** (pgdg repo) on Aeza only, bound to `127.0.0.1,10.88.0.1:5432` — NOT public. Cluster `16/main`, restart with `systemctl restart postgresql@16-main`. DB `proxyness` owned by role `proxyness` (not superuser). `pg_hba.conf` allows `10.88.0.2/32` + `127.0.0.1/32` via `scram-sha-256`. Connection URL lives in `/etc/proxyness/db.env` (mode 600) on both VPSs as `PROXYNESS_DB_URL=postgres://proxyness:<pw>@10.88.0.1:5432/proxyness?sslmode=disable`. `sslmode=disable` is deliberate — the WG link already encrypts, TLS on top would double-encrypt.
