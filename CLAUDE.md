...
- **One-off SQLite→Postgres migrator**: `server/cmd/migrate-pg/`. Reads a SQLite snapshot, inserts every row into Postgres preserving IDs via `pgx.CopyFrom`, then `setval`s each identity sequence to `MAX(id)`. Filters orphan FK rows (traffic_stats/site_domains/user_sites all have dangling references in Aeza's SQLite — SQLite ran with `PRAGMA foreign_keys=OFF`). The proxy-server runtime does NOT use this tool — it's a one-time migration. As of 2026-04-16, the `proxyness` DB on Aeza is populated from Aeza's SQLite but the live proxy container still writes to `/data/data.db` on both VPSs; Postgres is passive until plan 4 flips the switch.
