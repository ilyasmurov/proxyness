...
- **Slow-poll wait on ENETUNREACH (D1/D3)**: When `reconnectTransport()` exhausts its fast retry budget with `transport.IsNetworkUnreachable(err)` (ENETUNREACH anywhere in the error chain, plus a string-match fallback for `%v`-wrapped chains), the D1/D3 branches in BOTH `tun/engine.go` and `tunnel/tunnel.go` call `waitForNetwork()` instead of `stopLocked()`. `waitForNetwork` is a 15-second slow-poll loop with a **2-minute budget** (`maxSlowPollAttempts = 8`). It returns nil on recovery, `errReconnectStopped` on stop, a wrapped non-network error on a non-ENETUNREACH failure, or `errSlowPollBudgetExhausted` once the budget runs out — all three non-nil outcomes fall through to `stopLocked` with `lastError = "Connection lost, please reconnect"`, which the client's 2-second status poll then turns into a fresh `engine.Start` via `startReconnect`. The budget matters because the ORIGINAL no-timeout design assumed the OS would eventually restore routes on its own — but the helper owns several routes the kernel doesn't re-install (`route add -host <server> <gw>`, `-ifscope <origInterface>` bypass routes). When macOS silently flushes those (cause unconfirmed — plausibly reconfigd / scutil / short sleep-wake, IP on en0 unchanged), `IP_BOUND_IF=en0` has no matching route for any destination including proxyness.smurov.com (Aeza) AND the current tunnel server (Timeweb), so UDP writes return ENETUNREACH and TLS `protectedDialUDP("tcp", ...)` fails the same way. `waitForNetwork` was looping silently because the rescue it needed lives in the helper (`createTUN`), not in the kernel. The budget plus client auto-reconnect recreates the engine, which re-runs `connectAndCreate`, which re-runs the helper's `createTUN`, which reinstalls all routes — the exact path the user was doing manually. At slow-poll entry we also dump `netstat -rn` / `route print` via `transport.LogNetworkState` so the next incident carries its own post-mortem. D2 is intentionally NOT patched: its error isn't typically ENETUNREACH (it's "transport dead" from `Alive()`) and D3 always trips first in the sleep scenario anyway. The ENETUNREACH detection lives in `daemon/internal/transport/errors.go` as a shared helper.
