...
- **Slow-poll wait on ENETUNREACH (D1/D3)**: When `reconnectTransport()` exhausts its fast retry budget with `transport.IsNetworkUnreachable(err)` (ENETUNREACH anywhere in the error chain, plus a string-match fallback for `%v`-wrapped chains), the D1/D3 branches in BOTH `tun/engine.go` and `tunnel/tunnel.go` call `waitForNetwork()` instead of `stopLocked()`. `waitForNetwork` is a slow-poll loop on a ramped schedule (3/5/7/10/15/20/30/30s, 120s total budget). It returns nil on recovery, `errReconnectStopped` on stop, a wrapped non-network error on a non-ENETUNREACH failure, or `errSlowPollBudgetExhausted` once the budget runs out — all three non-nil outcomes fall through to `stopLocked` with `lastError = "Connection lost, please reconnect"`, which the client's 2-second status poll then turns into a fresh `engine.Start` via `startReconnect`. On each tick the loop calls the helper's new `refresh_routes` action (via `engine.RefreshRoutes`) before retrying — this is the **primary recovery path**, not the final budget-exhaust restart. Rationale: the first generation of this loop (commit `1f9d8b7`) just retried `tryReconnectOnce()` on each tick, which is purely a socket reconnect — it can't recover the actual failure mode. The observed failure mode is `sendto()` returning ENETUNREACH *immediately* despite the routing table showing valid routes: a darwin-specific stale neighbor cache / "!" blackhole on the gateway after an interface flap (Docker vmnetd re-creating bridged interfaces, USB-ethernet plug/unplug, brief wifi loss). The only thing that cleared it was the user manually clicking disconnect+connect, which runs the helper's `destroyTUN` → `createTUN` and the `route add -host <server> <gateway>` inside forces fresh neighbor resolution. `RefreshRoutes` does the route part of that cycle *without* tearing down TUN, so browsers/apps don't see their connections die. It deletes and re-adds the server host route, DNS routes, and `-ifscope <origInterface>` bypass routes (darwin) / physical-IF bypass routes (windows), and re-reads the default gateway/interface so a wifi↔ethernet switch works. Tunnel's waitForNetwork gets the callback via `SetRouteRefresher` wired in daemon/cmd/main.go — it's nil in tests and the call is skipped silently. At slow-poll entry we also dump `netstat -rn` / `route print` via `transport.LogNetworkState` and each tick calls `transport.LogNetworkDiagnostics` (arp + route get) so the next incident carries its own post-mortem. D2 is intentionally NOT patched: its error isn't typically ENETUNREACH (it's "transport dead" from `Alive()`) and D3 always trips first in the sleep scenario anyway. The ENETUNREACH detection lives in `daemon/internal/transport/errors.go` as a shared helper.
