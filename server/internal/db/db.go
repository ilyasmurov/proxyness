...
func Open(path string) (*DB, error) {
	...
	// Migrate old changelog table (version/date/changes → id/title/description/type/created_at)
...
func (d *DB) GetChangelog(page, perPage int) ([]ChangelogEntry, int, error) {
	...
	rows, err := d.sql.Query(
		`SELECT id, title, COALESCE(description,''), type, created_at FROM changelog ORDER BY created_at DESC LIMIT ? OFFSET ?`,
		perPage, offset,
	)
	if err != nil {
		return nil, 0, err
...
func (d *DB) GetChangelogUnseenCount(since string) (int, error) {
	...
	if since == "" {
		...
		err = d.sql.QueryRow(`SELECT COUNT(*) FROM changelog WHERE created_at > ?`, since).Scan(&count)
	}
	return count, err
}
